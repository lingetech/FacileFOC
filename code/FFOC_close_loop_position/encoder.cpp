...
//构造函数
Encoder::Encoder(int pin_SDA, int pin_SCL, int I2C_address, int high_register_address, int low_register_address, int direction):    //direction = 1：电机逆时针旋转，编码器数值增加；direction = -1：电机顺时针旋转，编码器数值增加
 pin_SDA(pin_SDA), pin_SCL(pin_SCL), I2C_address(I2C_address), high_register_address(high_register_address), low_register_address(low_register_address), direction(direction){
...
int Encoder::readData(){
  int buffer = NAN;
	...
	while(Wire.available() == 0);	//Wire.available()：检查是否有数据已经到达接收缓冲区；while(Wire.available() == 0);：阻塞程序，直到有可读的数据。
